...
def convolve(img, kernel, crop=True, return_fft=False, fftshift=True,
    ...
    *NOTE* Order matters; the kernel should be second.
    ...
    bigimg[int(imgquarter1x):int(imgquarter3x),int(imgquarter1y):int(imgquarter3y)] = img
    bigkernel[int(kernelquarter1x):int(kernelquarter3x),int(kernelquarter1y):int(kernelquarter3y)] = kernel 
    imgfft = fft2(bigimg)
    ...
    if ignore_nan or ignore_zeros:
        ...
        bigimwt[int(imgquarter1x):int(imgquarter3x),int(imgquarter1y):int(imgquarter3y)] = 1.0-nanmaskimg*ignore_nan
        wtfft = fft2(bigimwt)
    ...
    if return_fft: 
        if fftshift: # default on 
            if crop:
                return np.fft.fftshift(fftmult)[ int(imgquarter1x):int(imgquarter3x), int(imgquarter1y):int(imgquarter3y) ]
            else:
    ...
    if crop:
        result = rifft[ int(imgquarter1x):int(imgquarter3x), int(imgquarter1y):int(imgquarter3y) ].real
        return result
    else:
        return rifft.real
...
def correlate2d(im1,im2,**kwargs):
    ...
    return convolve(im1,im2[::-1,::-1],**kwargs)
