...
def pspec(psd2, return_index=True, wavenumber=False, return_stddev=False, azbins=1, binsize=1.0, view=True, **kwargs):
    """
    ...
    if return_stddev:
        ...
        #pyplot.loglog(freq,zz,label=label)
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        gray = y1[i].reshape(128,128)
        psd20 = PSD2(gray)
        psd1.append(np.array(pspec(psd20)).flatten())

        gray2 = y2[i].reshape(128,128)
        psd21 = PSD2(gray2)
        psd2.append(np.array(pspec(psd21)).flatten())

    df1 = pd.DataFrame(psd1)
    df2 = pd.DataFrame(psd2)

    pyplot.matshow(df2.corr())
    pyplot.colorbar()
    pyplot.show()
