...
if __name__ == "__main__":
    image = cv2.imread('camb.png')
    gray = cv2.cvtColor(image,cv2.COLOR_BGR2GRAY)
    image2 = cv2.imread('Test2.jpg')
    gray2 = cv2.cvtColor(image2,cv2.COLOR_BGR2GRAY)
    image3 = cv2.imread('camb2.png')
    gray3 = cv2.cvtColor(image3,cv2.COLOR_BGR2GRAY)
    image4 = cv2.imread('Test.jpg')
    gray4 = cv2.cvtColor(image4,cv2.COLOR_BGR2GRAY)
    psd20 = PSD2(gray,image2=gray3)
    label="CAMBxCAMB"
    pspec(psd20)
    psd21 = PSD2(gray,image2=gray2)
    label="CAMBxGAN"
    pspec(psd21)
    psd22 = PSD2(gray2,image2=gray4)
    label="GANxGAN"
    pspec(psd22)
    pyplot.legend()
    pyplot.show()